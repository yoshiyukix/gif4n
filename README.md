...
`npx expo run:ios --device` は Development Build のため Metro が必要です。
Metro なしで確認したい場合は、JS バンドルをアプリへ同梱した Release ビルドを使います。

### 方式 A: Xcode で Release 実行（ローカル）
...
または、Xcode を開かずに CLI で Release ビルドする場合：

```bash
npx expo run:ios --device --configuration Release
```

> **補足**: 既に Debug 版が端末に入っていると Metro 接続画面と混同しやすいため、必要に応じて一度アプリを削除してから再インストールしてください。

### 方式 B: EAS Build（配布・共有向け）
...
> **補足**: 開発中のデバッグは従来どおり `npx expo run:ios --device`（Metro あり）が最速です。最終確認だけ Release（Metro なし）を使う運用がおすすめです。
