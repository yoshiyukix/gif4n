...
| VideoThumbnailService  | `src/infrastructure/VideoThumbnailService.ts` | Visible Thumbnail の生成と ConvertingScreen 用プレビューサムネイル生成を一箇所に集約する                                                                    |
| useConversionSession   | `src/hooks/useConversionSession.ts`        | パイロット推定、開始プリセット決定、本変換、進捗反映、エラー分類、キャンセルを一つの session module として提供する                                          |
...
[動画選択]
  ...
  → HomeScreen の Visible Thumbnail は VideoThumbnailService.getVisibleThumbnail(...) で生成
  → TrimScreen のプレビュー・トリム UI 表示
  → TrimRange 指定（パイロット完了を待たず即時変換ボタンが有効）
  → [GIF動画に変換] 時に VideoThumbnailService.getConversionPreviewThumbnail(...) で thumbnailUri を生成
  → source / trimRange / thumbnailUri のみを ConvertingScreen へ渡す
  → ConvertingScreen 表示後、useConversionSession.start(source, trimRange) を呼び出し
       ...
       └─ NativeGifService.convert(source, trim, preset)
            ...
            └─ 進捗コールバック → useConversionSession の progressRate 更新
...
### `useConversionSession` hook

```typescript
function useConversionSession(options: {
...
// dependencies を省略した場合は NativeGifService / PilotEstimationUseCase /
// ConversionUseCase / outputSizeResolver を内部生成する
```
...
| ---------------- | ------------------------------------------------------------ | -------------------------------------------------------------- |
| HomeScreen       | expo-media-library, expo-document-picker, VideoThumbnailService | VideoAssetReference / file request / Visible Thumbnail         |
| PrepareVideoScreen | VideoSourcePreparationUseCase                               | 準備中状態 / VideoSource preparation request                   |
| TrimScreen       | useTrim, VideoThumbnailService                               | VideoSource / TrimRange / thumbnailUri                         |
| ConvertingScreen | useConversionSession, useSettings                            | progressRate / status（piloting/running/done/error/cancelled） |
...
- `useConversionSession` が ConvertingScreen 遷移直後に `PilotEstimationUseCase.run()` を実行し（status: `'piloting'`）、実測 bytes/sec から最適プリセットを決定してから本変換を開始する（status: `'running'`）。TrimScreen で待機なし
