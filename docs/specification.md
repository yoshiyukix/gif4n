...
| useReviewPrompt        | `src/hooks/useReviewPrompt.ts`             | Conversion Success Count と Review Prompt Attempt を永続化し、レビュー促進の表示条件を判定する wiring hook                                                   |
| AppNavigator           | `src/navigation/AppNavigator.tsx`          | ルートナビゲーター（StudioNavigator をラップ）                                                                                                               |
| StudioNavigator        | `src/navigation/StudioNavigator.tsx`       | メインスタック（Home → Trim → Converting → Result）+ Settings モーダル導線                                                                                   |
...
│   │   └── useReviewPrompt.ts         ← レビュー促進状態の永続化と表示条件判定
│   ├── navigation/
│   │   ├── AppNavigator.tsx
│   │   ├── StudioNavigator.tsx
│   │   └── types.ts
...
フォトライブラリ上の動画を指す軽量な参照。HomeScreen から TrimScreen へ即時に渡すために使う。
...
```
[動画選択]
  → HomeScreen は VideoAssetReference を TrimScreen へ即時に渡す
  → TrimScreen は準備中表示を出し、VideoImportService.importAsset(...) で MediaLibrary の URI を正規化してアプリの cacheDirectory へコピー
  ...
  → ResultScreen 表示
       └─ useReviewPrompt.recordConversionSuccess() で Conversion Success Count を加算
       └─ Conversion Success Count >= 3 かつ Review Prompt Attempt 未実施なら、
          初回描画後の次フレームで expo-store-review.isAvailableAsync() を確認し、
          利用可能な環境でのみ expo-store-review.requestReview() を一度だけ試行
...
```typescript
interface UseReviewPromptResult {
  recordConversionSuccess: () => Promise<number>;
  shouldAttemptReviewPrompt: () => Promise<boolean>;
  markReviewPromptAttempted: () => Promise<void>;
}

function useReviewPrompt(): UseReviewPromptResult;
// AsyncStorage キー: '@gif_to_note/review_prompt'
// Conversion Success Count と Review Prompt Attempt を管理する
```
...
| ---------------- | ------------------------------------------------------------ | -------------------------------------------------------------- |
| HomeScreen       | expo-media-library, expo-document-picker, VideoImportService | VideoAssetReference / ファイル選択時の VideoSource             |
| TrimScreen       | VideoImportService, useTrim                                  | 準備中状態 / VideoSource / TrimRange                           |
| ConvertingScreen | useConversionProcess, useSettings                            | progressRate / status（piloting/running/done/error/cancelled） |
| ResultScreen     | useMediaActions, useReviewPrompt, expo-store-review          | outputUri / outputSizeBytes / preset / ReviewPromptState       |
