...
| usePilotEstimation   | `src/hooks/usePilotEstimation.ts`     | TrimScreen でパイロット変換を実行し bytes/sec を提供する hook                                                |
| useTrim              | `src/hooks/useTrim.ts`                | トリミング状態管理。`MAX_TRIM_DURATION_SEC`（15 秒）を上限として制限し、範囲が超えた場合は反対側のハンドルを連動して縮める                                                                                         |
...
| フィールド  | 型             | デフォルト | 説明                               |
| ----------- | -------------- | ---------- | ---------------------------------- |
| `maxSizeMb` | `5 \| 7 \| 10` | `10`       | GIF 変換の最大ファイルサイズ（MB） |
...
トリミング範囲。`endSec - startSec` は最大 `MAX_TRIM_DURATION_SEC`（15 秒）に制限される。
